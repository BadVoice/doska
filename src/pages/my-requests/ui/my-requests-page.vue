  ...
  <template v-else>
    <div class="w-full">
      <div
        ...
        <h3 class="text-[18px] font-semibold" v-else-if="selectedSortType < -4">
          Мои возвраты
        </h3>
        <h3 class="text-[18px] font-semibold" v-else>Мои заявки</h3>
        <Button
          v-if="selectedSortType !== -3 && selectedSortType !== -4"
    ...
    <div
      ...
      <div
        ...
        v-if="!filteredList?.length && !pending && selectedSortType >= -2">
        <img
      ...
      <ScrollArea class="h-[calc(100vh-186px)]">
        ...
        <DynamicScroller
          v-if="
            (selectedSortType === -3 || selectedSortType === -4) &&
            !pendingOrders &&
            orders?.data
          "
          :items="
            (selectedCompany?.id
              ...
              selectedSortType === -4 ? o.status === 1 : o.status === 0,
            )
          "
          :min-item-size="148"
        ...
        <Pagination
          v-if="!pendingOrders && (orders?.data?.results?.length ?? 0) > 150"
          v-slot="{ page }"
          :total="filteredList?.length"
          :sibling-count="0"
          show-edges
